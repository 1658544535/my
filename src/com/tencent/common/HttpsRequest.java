...
public class HttpsRequest implements IServiceRequest {
  ...
  private void init() throws IOException, KeyStoreException, UnrecoverableKeyException,
    ...
    SSLContext sslcontext =
        SSLContexts.custom().loadKeyMaterial(keyStore, Configure.getCertPassword().toCharArray())
            .build();
  ...
  private void jsinit() throws IOException, KeyStoreException, UnrecoverableKeyException,
      NoSuchAlgorithmException, KeyManagementException {

    KeyStore keyStore = KeyStore.getInstance("PKCS12");
    FileInputStream instream = new FileInputStream(new File(Configure.getJscertLocalPath()));//加载本地的证书进行https加密传输
    try {
      keyStore.load(instream, Configure.getJscertPassword().toCharArray());//设置证书密码
    } catch (CertificateException e) {
      e.printStackTrace();
    } catch (NoSuchAlgorithmException e) {
      e.printStackTrace();
    ...
    // Trust own CA and all self-signed certs
    SSLContext sslcontext =
        SSLContexts.custom().loadKeyMaterial(keyStore, Configure.getJscertPassword().toCharArray())
            .build();
    // SSLContext sslcontext = SSLContexts.createDefault();
    // Allow TLSv1 protocol only
    SSLConnectionSocketFactory sslsf =
  ...
  public String sendPost(String url, Object xmlObj) throws IOException, KeyStoreException,
    ...
    postDataXML = postDataXML.replace("<wxtrade>APP</wxtrade>\n", "");
  ...
  @Override
  public String sendJSPost(String url, Object xmlObj) throws IOException, KeyStoreException,
      UnrecoverableKeyException, NoSuchAlgorithmException, KeyManagementException {
    ...
    //将要提交给API的数据对象转换成XML格式数据Post给API
    String postDataXML = xStreamForRequestPostData.toXML(xmlObj);
    postDataXML = postDataXML.replace("<wxtrade>JSAPI</wxtrade>\n", "");
    Util.log("API，POST过去的数据是：");
    Util.log(postDataXML);
