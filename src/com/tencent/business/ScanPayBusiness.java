...
public class ScanPayBusiness {
  ...
  public ScanPayBusiness() throws IllegalAccessException, ClassNotFoundException,
  ...
  public void run(ScanPayReqData scanPayReqData, ResultListener resultListener) throws Exception {
    ...
    ScanPayResData scanPayResData =
        (ScanPayResData) Util.getObjectFromXML(payServiceResponseString, ScanPayResData.class);

    // 异步发送统计请求
    // *

    ReportReqData reportReqData =
        new ReportReqData(scanPayReqData.getDevice_info(),
            Configure.PAY_API,
            ...
            scanPayResData.getErr_code_des(), scanPayResData.getOut_trade_no(),
    ...
    if (scanPayResData.getReturn_code().equals("FAIL")) {
      ...
      if (scanPayResData.getResult_code().equals("SUCCESS")) {
        ...
        if (errorCode.equals("AUTHCODEEXPIRE") || errorCode.equals("AUTH_CODE_INVALID")
  ...
  private boolean doOnePayQuery(String outTradeNo, ResultListener resultListener) throws Exception {
    ...
    ScanPayQueryResData scanPayQueryResData =
        (ScanPayQueryResData) Util.getObjectFromXML(payQueryServiceResponseString,
   ...
   */
  private boolean doPayQueryLoop(int loopCount, String outTradeNo, ResultListener resultListener)
      throws Exception {
    // 至少查询一次
    if (loopCount == 0) {
      loopCount = 1;
  ...
  private boolean doOneReverse(String outTradeNo, ResultListener resultListener) throws Exception {
    ...
    // 将从API返回的XML数据映射到Java对象
    ReverseResData reverseResData =
        (ReverseResData) Util.getObjectFromXML(reverseResponseString, ReverseResData.class);
