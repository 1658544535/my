...
public class JPushUtils {
  ...
  private static final String appKey = "8ffb1532d111f064c4e06861";
  private static final String masterSecret = "6535c2f180d5199b4a32cd80";
  ...
  // extras.put("groupID", "34");
  // extras.put("image", "http://pin.taozhuma.com/upfiles/product/20160929102508297535.jpg");
  // extras.put("title", "最新订单来自pdh11**66，一起拼");
  // customerMsgJPush("", extras);
  //}

  public static void customerMsgJPush(String content, Map<String, String> extras) {
    PushPayload payload = buildPushObject_ios_audienceMore_messageWithExtras(content, extras);
    sendPush(payload);
  }
  ...
  public static void testSendPush_fromJSON() {
    ...
    JPushClient jpushClient = new JPushClient(masterSecret, appKey, null, clientConfig);
    Gson gson =
        new GsonBuilder().registerTypeAdapter(PlatformNotification.class,
                ...
                Notification
                    ...
                    .addPlatformNotification(
                        IosNotification.newBuilder().incrBadge(1)
                            .addExtra("extra_key", "extra_value").build()).build()).build();
  ...
  public static void testSendIosAlert() {
    ...
    IosAlert alert =
        IosAlert.newBuilder().setTitleAndBody("test alert", "subtitle", "test ios alert json")
            .setActionLocKey("PLAY").build();
    try {
      PushResult result =
          jpushClient.sendIosNotificationWithAlias(alert, new HashMap<String, String>(), "alias1");
      LOG.info("Got result - " + result);
    } catch (APIConnectionException e) {
      LOG.error("Connection error. Should retry later. ", e);
    } catch (APIRequestException e) {
  ...
  public static void testSendWithSMS() {
    ...
    try {
      ...
      PushResult result =
          jpushClient.sendAndroidMessageWithAlias("Test SMS", "test sms", sms, "alias1");
      LOG.info("Got result - " + result);
    } catch (APIConnectionException e) {
      LOG.error("Connection error. Should retry later. ", e);
    } catch (APIRequestException e) {
