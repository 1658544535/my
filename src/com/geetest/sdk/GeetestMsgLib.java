...
public class GeetestMsgLib extends GeetestLib {
   ...
   * 
   * @see com.geetest.sdk.java.GeetestLib#resquestIsLegal(javax.servlet.http. HttpServletRequest)
   */
  @Override
  ...
  public int sendMsgCodeReq(HttpServletRequest request) {
    ...
    String query =
        String.format("seccode=%s&sdk=%s&phone=%s&msg_id=%s", seccode, this.sdkLang + "_"
            + this.verName, phoneNum, this.getCaptchaId());
    gtlog(query);
  ...
  public static GeetestMsgLib getGtMsgSession(HttpServletRequest request) {
    return (GeetestMsgLib) request.getSession().getAttribute(gt_msg_session_key);
  }
  ...
  public int validateMsgCode(HttpServletRequest request) throws Exception {
    ...
    String query =
        String.format("phone=%s&code=%s&msg_id=%s", phoneNum, msgCode, this.getCaptchaId());
    String response = "";
