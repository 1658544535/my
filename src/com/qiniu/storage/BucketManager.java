...
public final class BucketManager {
  ...
  /**
   * EncodedEntryURI格式 当 mustHaveKey 为 false， 且 key 为 null 时，返回 urlsafe_base64_encode(Bucket);
   ...
   * @return FileInfo迭代器
   */
  public FileListIterator createFileListIterator(String bucket, String prefix, int limit,
      String delimiter) {
   ...
   */
  public FileListing listFiles(String bucket, String prefix, String marker, int limit,
      String delimiter) throws QiniuException {
    StringMap map =
        new StringMap().put("bucket", bucket).putNotEmpty("marker", marker)
            .putNotEmpty("prefix", prefix).putNotEmpty("delimiter", delimiter)
