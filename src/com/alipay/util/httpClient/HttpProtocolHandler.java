...
public class HttpProtocolHandler {
  ...
  private int defaultMaxConnPerHost = 15;

  private int defaultMaxTotalConn = 40;
  ...
  public HttpResponse execute(HttpRequest request, String strParaFileName, String strFilePath)
    ...
    if (request.getMethod().equals(HttpRequest.METHOD_GET)) {
      ...
      for (int i = 0; i < request.getParameters().length; i++) {
        parts.add(new StringPart(request.getParameters()[i].getName(), request.getParameters()[i]
            .getValue(), charset));
      }
