...
<body>
    <div class="wraper">
    	<%--
    	<div class="w-title">
    		<h3 class="title">${knowledgeBasePojo.title}</h3>
    		<div class="info"></div>
    	</div>
		--%>
    	<div class="w-img">
    		<%--<div class="img"><img src="<%=request.getContextPath() %>/upfiles/knowledgeBase/${knowledgeBasePojo.smallIcon}" /></div>--%>
    		<div class="info">${knowledgeBasePojo.content}</div>
    	</div>
    	
    	<div id="w-btn" class="w-btn youAnniu">
    		<a href='{"type":"age","value":"${fn:trim(knowledgeBasePojo.ageType)}"}'>#${knowledgeBasePojo.ageTypeName}</a> 
    		<a href='{"type":"skill","value":"${fn:trim(knowledgeBasePojo.skillType)}"}'>#${knowledgeBasePojo.skillTypeName}</a> 
    		<a href='{"type":"product","value":"${fn:trim(knowledgeBasePojo.productType)}"}'>#${knowledgeBasePojo.productTypeName}</a>
    	</div>
    </div>
    ...
    <script>
        ...
        $(function(){
        	//console.log("${knowledgeBasePojo.yourFavouritesId}");
        	if("${knowledgeBasePojo.yourFavouritesId}" != null && "${knowledgeBasePojo.yourFavouritesId}" != ""){
	        	$.ajax({
				      type: "get",
				      url: "getYourFavouritesInfo.do?id="+"${knowledgeBasePojo.yourFavouritesId}" ,
				      dataType: 'json',
