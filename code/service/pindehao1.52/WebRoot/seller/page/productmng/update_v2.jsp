...
<script src="<s:i18n name="sysconfig"><s:text name="seller_dns" /></s:i18n>/seller/js/fileupload/jquery.ui.widget.js" type="text/javascript"></script>
<script src="<s:i18n name="sysconfig"><s:text name="seller_dns" /></s:i18n>/seller/js/fileupload/jquery.iframe-transport.js" type="text/javascript"></script>
<script src="<s:i18n name="sysconfig"><s:text name="seller_dns" /></s:i18n>/seller/js/fileupload/jquery.fileupload.js" type="text/javascript"></script>

    <title>商家中心</title>
    </head>
	<body>
<jsp:include page="../sellerHeader.jsp"></jsp:include>
<form class="ui-form product-add-form"  action="productUpdateSeller.do?productPojo.userId=${productPojo.userId}" method="post" id="from1" enctype="multipart/form-data" onkeydown="if(event.keyCode==13)return false;">	
...
<!--                                                     <a href="javascript:;" class="view-WhereToShow">
	                                    ...
	                                    <div class="uploadify"style="position:relative;height: 140px; width: 140px;margin-top: 10px;">
                                         ...
                                         <p class="uploadPreview_note" style="display:none;"><i class="iconfont">&#xf00f7;</i>更改图片</p>
                                         <div class="uploadPreview_img">
                                         	<img src="<s:i18n name="sysconfig"><s:text name="seller_dns" /></s:i18n>/upfiles/product/<%=images[n+1] %>"  style="width:120px;height:120px;" />
...
<!--                                 <a class="promise-detail" href="#" target="_blank">
		...
		<div class="m-fb-dialog J_fb-dialog">
                <div class="title">
                    意见反馈
                </div>
                <div class="mnc">
                    ...
                    <textarea class="control-text J_fb-text" placeholder="请给我们提意见哟，么么哒！"></textarea>
                </div>
                <div class="footer">
                    <button class="btn btn-sure">
                        确定
                    </button>
...
<script>
	...
	$(function(){
		...
		//弹出自定义框
		$(".view-ShowSellPropCustomBoxBtn").on("click",function(){
			if($(".sellpropCustomListItem li").length > 0){
				$(".view-SellpropCustomList").show();
				$(".view-SellpropCustomNote").hide();
			}else{
				$(".view-SellpropCustomList").hide();
				$(".view-SellpropCustomNote").show();
			}
			$(".showSellPropCustomBox,.miniDialog_mask").show();
		});
		//保存
		$(".view-SellpropCustomSaveBtn").on("click",function(){
			$(".sellpropCustomListItem li").each(function(index, element) {
				var temId = $(element).find("input").attr("data-id"),
					temValue = $(element).find("input").val();
				for(var i=0; i<$(".product-add-checkbox>li").length; i++){
					var temId2 = $(".product-add-checkbox>li").eq(i).find("input").attr("data-id"),
						temValue2 = $(".product-add-checkbox>li").eq(i).find("input").val();
					if(temId2 == temId){
						$(".product-add-checkbox>li").eq(i).find("input").attr("data-value",temValue);
						$(".product-add-checkbox>li").eq(i).find("input").next().html(temValue);
					}
				}
	...
	var productTypeIds =new tt.Field(" 商品二级类目","productPojo.productTypeIds").setMsgId("productTypeId_mgId");
