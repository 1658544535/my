...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	function del(val) {
		if(confirm("你真的想删除该记录么？")) {
			var url = "deleProductImagesSellerWeb.do?productImagesPojo.productId=<s:property value='productImagesPojo.productId'/>&productImagesPojo.id="+val;	
			doOpreator(url,null);
		}else{
			return ;
		}
	...
	function goRefreshPage(result){
		...
		if(result=="1"){
			alert("删除成功");
			queryData("getCountProductImagesSellerWeb.do?productImagesPojo.productId=${productImagesPojo.productId}", "getProductImagesAllSellerWeb.do?productImagesPojo.productId=${productImagesPojo.productId}&page.rowCount=${page.rowCount}&randdelete="+rand,pageSize);
		}else{
			alert("删除失败");
		}
	}
	...
	function check(val) {
		if(confirm("确认要通过审核吗？")) {
			var url = "checkProductImagesSellerWeb.do?productImagesPojo.productId=<s:property value='productImagesPojo.productId'/>&productImagesPojo.id="+val;	
	...
	function uncheck(val) {
		if(confirm("确认要取消审核吗？")) {
			var url = "unCheckProductImagesSellerWeb.do?productImagesPojo.productId=<s:property value='productImagesPojo.productId'/>&productImagesPojo.id="+val;	
	...
	/*批量审核*/
	function checkAll() {
		$("#idform").attr("action","productImagescheckAllSellerWeb.do?productImagesPojo.productId=<s:property value='productImagesPojo.productId'/>");
		$("#idform").submit();
	}
	</script>
	<body>
                    ...
                    <div class="pure-u-1 main">
                        ...
                        <h1 class="seller-title">
                            ...
                            <div class="seller-title-head-more">
                                ...
                                <a class="ui-button ui-button-lgreen" href="goproductImagesAddSellerWeb.do?productImagesPojo.productId=<s:property value='productImagesPojo.productId'/>" >
                        ...
                        <div class="seller-search-form" style="margin: 8px 0 -14px -8px;">
                            <form action="#" method="post" accept-charset="utf-8" id="sysform">
                                <div class="ui-box-content">
                                    ...
                                    <div class="ui-form-item">
                                        <input class="ui-input w-mart-input" type="text" name="productImagesPojo.images" value="" placeholder="请输入图片名称" style="width:150px; " id="images">
                                    </div>
                        ...
                        <form action="productImagesDeleteIdSellerWeb.do?productImagesPojo.productId=<s:property value='productImagesPojo.productId'/>&productImagesPojo.userId=${productImagesPojo.userId}" id="idform" method="post">
...
<script type="text/javascript">
	...
	function query() {
			var rand=Math.random() * ( 100000 + 1);
			queryData("getCountProductImagesSellerWeb.do?productImagesPojo.productId=<s:property value='productImagesPojo.productId'/>", "getProductImagesAllSellerWeb.do?productImagesPojo.productId=<s:property value='productImagesPojo.productId'/>&randquery="+rand,pageSize);
	}
	...
	function installPage() {
		$("#body").append(
				...
				"<td><img src='<s:i18n name="sysconfig"><s:text name="seller_dns" /></s:i18n>/upfiles/product/"+ this.images + "' height='100px' width='100px' /></td>"+
				"<td>"+ this.images + "</td>"+
				"<td>"+ this.sorting + "</td>"+
				"<td>"+ this.statusName + "</td>"+
				"<td><a href='javascript:;' class='del_btn' onclick=del('"+this.id+"')>删除</a></br>"+
	//			"<a href='javascript:;' class='edit_btn' onclick=check('"+this.id+"')>审核</a></br>"+
	//			"<a href='javascript:;' class='edit_btn' onclick=uncheck('"+this.id+"')>取消审核</a></br>"+			
				"<a class='edit_btn' href='goFindProductImagesSellerWeb.do?productImagesPojo.id="+this.id +"'>编辑</a></td></tr>");
	}
	...
	$(function() {
		 ...
		 MAOWU.page.init(<s:property value='page.rowCount'/>,
				 "getProductImagesAllSellerWeb.do?productImagesPojo.productId=<s:property value='productImagesPojo.productId'/>&randIni="+rand,pageSize);
