...
<body>
	<div class="sub_wrap">
		<div class="s_nav">
			<a href="#">商家中心管理</a> &gt; <a href="#">品类推荐</a>&gt; <a href="#">添加品类推荐</a>
		</div>
			...
			<div id="search_show" style="">
				<table width="100%" border="0" class="Search_table">
					<tr>
						...
						<td><select name="productTypePojo.topLevel"  class="floatLeft" onchange="setSecond(this)">		
					    <option value ="">     --请选择--</option>
						<c:forEach items="${productTypeList}" var="productTypePojo">
						<option value ="${productTypePojo.id}">${productTypePojo.name}</option>
						</c:forEach>
						</select></td>
					    <td align="right">二级分类：</td>
					    <td><select id="second" name="productTypePojo.id" class="floatLeft"></select>	</td>
					</tr>
...
<script type="text/javascript">
	...
	function query() {
		if (tt.validate()) {
			var rand = Math.random() * (100000 + 1);
			queryData("getCountProductTypeThree.do", "getProductTypeThree.do?randquery=" + rand);
		}
	}
		...
		$("#body")
				.append(
						<!--"<tr><td><input  name='tids' type='checkbox' value="+this.id +"   /></td><td>"-->
								"<tr><td>"+ this.name
								+ "</td><td>"
								+ "<img src='<s:i18n name="sysconfig"><s:text name="houtai_dns" /></s:i18n>/upfiles/productType/"+ this.image + "' height='50px' />"
								+ "</td><td>"
								+ "三级"
								+ "</td><td>"
								+ this.parentName
								+ "</td><s:if test="#session.role.roleId!=7 && panduan!=1"><td><a class='edit_btn' href='selectProductTypeRecommend.do?productTypeRecommendPojo.productTypeId="+this.id +"'>选取</a></td></s:if><s:if test="#session.role.roleId!=7 && panduan==1"><td><a class='edit_btn' href='addCategoryDetailSettingListOk.do?categoryDetailSettingPojo.categoryId=${categoryId}&categoryDetailSettingPojo.typeId="+this.id +"'>选取${categoryDetailSettingPojo.categoryId}</a></td></s:if></tr>");   
	}
	...
	$(function() {
		...
		MAOWU.page.init(<s:property value="page.rowCount"/>,
				"getProductTypeThree.do?randIni=" + rand);
