...
<script type="text/javascript">
	...
	function del(val) {
		if (confirm("你真的想删除么？")) {
			...
			var url = "delShopRecommend.do?shopRecommendPojo.id=" + val;
			doOpreator(url, null);
		} else {
			return;
		}
	...
	function goRefreshPage(result) {
		...
		if (result =="1") {
			...
			queryData("getShopRecommendCount.do", "shopRecommendAllList.do?randquery=" + rand);
		} else {
			alert("删除失败");
		}
	}
...
<body>
	<div class="sub_wrap">
		<div class="s_nav">
			<a href="#">创客管理</a> &gt; <a href="#">店铺推荐</a>
		</div>
			<form action=goShopRecommend.do method="post" id="sysform">
		...
		<div>
			...
			<a class="Add_btn" href="userMakerShop.do?panduan=1">新增推荐</a></s:if>
			<form action="shopRecommendDeleteId.do" id="idform" method="post">
...
<script type="text/javascript">
		...
		$("#body")
				.append(
						<!--"<tr><td><input  name='tids' type='checkbox' value="+this.id +"   /></td><td>"-->
								"<tr><td>"+ this.name
								+ "</td><td>"
								+ "<img src='<s:i18n name="sysconfig"><s:text name="houtai_dns" /></s:i18n>/upfiles/shop/"+ this.images + "' height='50px' />"
								+ "</td><td>"
								+ this.sorting
								+ "</td><td>"
								+ this.createDateStr
								...
								+ this.id
								+ "')>删除</a>"
								+ "<a class='edit_btn' href='goUpdateShopRecommend.do?shopRecommendPojo.id="
	...
	$(function() {
		...
		MAOWU.page.init(<s:property value="page.rowCount"/>,
				"shopRecommendAllList.do?randIni=" + rand);
