...
<body>
<div class="sub_wrap">
<div class="s_nav"><a href="#">标签管理</a> &gt; <a href="favouritesLabelList.do">有你喜欢</a> &gt; <a href="#">查看</a></div>
  ...
  <div class="h15"></div>
  <div>
<a class="Add_btn" href="userCirclePostFavouritesAdd.do?yourFavouritesPojo.id=${yourFavouritesPojo.id }&yourFavouritesPojo.contentType=${yourFavouritesPojo.contentType }" >添加</a>
<span name="isDelete"><a class="Add_btn" onclick="deleteAll()">提交</a></span>
...
function query() {
	if(tt.validate()){
		var rand=Math.random() * ( 100000 + 1);
		queryData("userCirclePostFavouritesCount.do", "userCirclePostFavouritesList.do?randquery="+rand);
	}
	...
	function installPage() {
		$("#body").append(
				...
				"<td>"+ this.sorting + "</td>"+
				"<td><a class='del_btn' onclick=updateSorting('"+this.favId+"','"+this.sorting+"')>改序号</a>"+
				"<input  name='tids' type='checkbox' value="+this.favId +" /><font name='tidsName'>选取</font></td>");
	}
	$(function() {
		/**
		  首次要初始化分页
...
function updateSorting(id,sorting){
	var msg = "当前的序号为："+sorting+"，请输新的序号！~";
	var newSorting = prompt(msg, "");
    if (!isNaN(newSorting) && newSorting.length <= 10){
    	var url = "doPostFavouritesUpdateSorting.do?yourFavouritesDetailPojo.id="+id+"&yourFavouritesDetailPojo.sorting="+newSorting;	
		doupdateSorting(url,null);
    }else{
    	alert("您输入的序号格式不正确！~请重新输入！~");
    }
}

function doupdateSorting(url,params){
	MAOWU.ajax.get(url, params, goupdateSortingRefreshPage);
}
 
function goupdateSortingRefreshPage(result){
	var rand = Math.random() * ( 100000 + 1);
	if(result == "1"){
		alert("修改成功！~");
		queryData("userCirclePostFavouritesCount.do", "userCirclePostFavouritesList.do?randquery="+rand);
