				...
				<table width="100%" border="0" class="Search_table">
					<tr>
						...
						<td><label><input type="text"
								name="order.orderNo" id="orderNo"
								value="${order.orderNo }"></label>
						</td>
						<td align="right">商品ID：</td>
						<td><label><input type="text"
								name="order.productId" id="productId"
								value="${order.productId }"></label>
					    </td>
					    <td align="right">收货人：</td>
						<td><label><input type="text"
								name="order.consignee" id="consignee"
								value="${order.consignee }"></label>
						</td>
					    <td align="right">收货人电话：</td>
						<td><label><input type="text"
								name="order.consigneePhone" id="consigneePhone"
								value="${order.consigneePhone }"></label>
						</td>
					</tr>
					<tr>	
						<td align="right">支付状态：</td>
						<td><select name="order.payStatus" id="payStatus" class="floatLeft">
						...
						<%-- <td><select name="order.isRefund" id="isRefund" class="floatLeft">
									<option value="">----请选择----</option>
						...
						<td><select name="order.refundStatus" id="refundStatus" class="floatLeft">
									<option value="">----请选择----</option>
						...
						<td><label><input type="text"
								name="order.logisticsNo" id="logisticsNo"
					...
					<tr>	
						...
						<td><label><input type="text"
								name="order.loginname" id="loginname"
								value="${order.loginname }"></label>
						</td>
						<td align="right">团编号：</td>
						<td><label><input type="text"
								name="order.attendId" id="attendId"
								value="${order.attendId }"></label>
					    </td>
						<td align="right">订单来源：</td>
						<td>
							<select name="order.source" id="source" class="floatLeft">
									<option value="">----请选择----</option>
						...
						<td>
								<select name="order.overdue" id="overdue" class="floatLeft"">
										<option value="">----请选择----</option>
					...
					<tr>
						...
						<td><input value="${order.beganday }" name="order.beganday" readonly="readonly" class="Wdate" type="text" onfocus="WdatePicker({ dateFmt: 'yyyy-MM-dd HH:mm', isShowToday: false, isShowClear: true })" id="beganday"/></td>
						<td><input value="${order.endday }" name="order.endday" readonly="readonly" class="Wdate" type="text" onfocus="WdatePicker({ dateFmt: 'yyyy-MM-dd HH:mm', isShowToday: false, isShowClear: true })" id="endday"/></td> 
	                    <td>                      
						...
						<td>
							<input value="${order.groupBeginDateStr }" name="order.groupBeginDateStr" readonly="readonly" class="Wdate" type="text" onfocus="WdatePicker({ dateFmt: 'yyyy-MM-dd HH:mm:ss', isShowToday: false, isShowClear: true })" id="groupBeginDateStr"/>
						</td>
						<td>
							<input value="${order.groupEndDateStr }" name="order.groupEndDateStr" readonly="readonly" class="Wdate" type="text" onfocus="WdatePicker({ dateFmt: 'yyyy-MM-dd HH:mm:ss', isShowToday: false, isShowClear: true })" id="groupEndDateStr"/>
						</td>
						</c:if>
					</tr>
					<tr>
						...
						<td><input value="${order.beganSendDate }" name="order.beganSendDate" readonly="readonly" class="Wdate" type="text" onfocus="WdatePicker({ dateFmt: 'yyyy-MM-dd HH:mm:ss', isShowToday: false, isShowClear: true })" id="beganSendDate"/></td>
						<td><input value="${order.endSendDate }" name="order.endSendDate" readonly="readonly" class="Wdate" type="text" onfocus="WdatePicker({ dateFmt: 'yyyy-MM-dd HH:mm:ss', isShowToday: false, isShowClear: true })" id="endSendDate"/></td>
					    </td>
						</s:if>	 
						<td align="right">24小时未发货：</td>
						<td>
							<select name="order.notShip" id="notShip" class="floatLeft">
									<option value="">----请选择----</option>
									<option value="1" >是</option>
							</select>
					    </td>   
					    <%-- <td align="right">开团时间：</td>
						<td><input value="${order.beganday1 }" name="order.beganday1" readonly="readonly" class="Wdate" type="text" onfocus="WdatePicker({ dateFmt: 'yyyy-MM-dd HH:mm', isShowToday: false, isShowClear: true })" id="beganday1"/></td>
						<td><input value="${order.endday1}" name="order.endday1" readonly="readonly" class="Wdate" type="text" onfocus="WdatePicker({ dateFmt: 'yyyy-MM-dd HH:mm', isShowToday: false, isShowClear: true })" id="endday1"/></td> --%> 
					</tr>
					<tr>
...
function query() {
	if($("#orderNo").val()!="" ||$("#productId").val()!=""||$("#consignee").val()!=""||$("#consigneePhone").val()!=""||$("#payStatus").val()!=""||$("#orderStatus").val()!=""||$("#refundStatus").val()!=""||$("#logisticsNo").val()!=""||$("#loginname").val()!=""||$("#attendId").val()!=""||$("#source").val()!=""||$("#overdue").val()!=""||$("#beganday").val()!=""||$("#endday").val()!=""||$("#groupBeginDateStr").val()!=""||$("#groupEndDateStr").val()!=""||$("#beganSendDate").val()!=""||$("#endSendDate").val()!=""||$("#notShip").val()!=""||$("#pdkLoginname").val()!=""||$("#suserId").val()!=""){
		...
		queryData("getQueryOrder.do?os=<s:property value='os'/>&a=<s:property value='a'/>&orderType=<s:property value='orderType'/>&order.userId=${order.userId}", "getQueryOrderList.do?os=<s:property value='os'/>&a=<s:property value='a'/>&orderType=<s:property value='orderType'/>&order.userId=${order.userId}&randquery="+rand);
			...
			pageNumber = 1;
		var rand=Math.random() * ( 100000 + 1);
		queryData("getQueryOrder.do?os=<s:property value='os'/>&b=1&a=<s:property value='a'/>&orderType=<s:property value='orderType'/>&order.userId=${order.userId}", "getQueryOrderList.do?os=<s:property value='os'/>&b=1&a=<s:property value='a'/>&orderType=<s:property value='orderType'/>&order.userId=${order.userId}&randquery="+rand);
	}
}
   //当前时间
    var now= new Date();
	...
	$(function() {
		 ...
		 MAOWU.page.init(<s:property value="page.rowCount"/>,
				"getQueryOrderList.do?b=1&randIni="+rand);
