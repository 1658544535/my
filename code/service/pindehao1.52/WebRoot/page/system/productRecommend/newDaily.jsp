...
function del(val){
		if(confirm("你真的想取消该记录么？")){
			var url = "delNewDailyOne.do?productRecommendPojo.id="+val;	
			doOpreator(url,null);
		}else{
			return ;
		}
	...
	function goRefreshPage(result){
		...
		if(result=="1"){
			alert("取消成功");
			queryData("goNewDailyCount.do", "goNewDailyList.do?randquery="+rand);
		}else{
			alert("取消失败");
		}
	}
...
<body>
	...
	<div class="s_nav"><a href="#">活动商品管理</a> &gt; <a href="#">每日上新</a></div>
	</c:if>
	<c:if test="${productRecommendPojo.type == 2 }">
	<div class="s_nav"><a href="#">活动商品管理</a> &gt; <a href="#">销量排行</a></div>
	</c:if>
	<!-- <div class="Search_control">
		...
		<form action="" method="post" id="sysform">
			<input type="hidden" name="productRecommendPojo.type" value="${productRecommendPojo.type }">
  ...
  <div class="h15"></div>
  <div>
<a class="Add_btn" href="goNewDailyAdd.do?productRecommendPojo.type=${productRecommendPojo.type }" >添加</a>
...
function query() {
	...
	function installPage() {
		$("#body").append(
				...
				"<a class='del_btn' onclick=updateSorting('"+this.id+"','"+this.sorting+"')>改序号</a>"+
				"<a class='del_btn' onclick=del('"+this.id+"')>取消</a></td></s:if>");
	}
	$(function() {
		/**
		  首次要初始化分页
		...
		MAOWU.page.init(<s:property value="page.rowCount"/>,
				"goNewDailyList.do?randIni="+rand);
...
function updateSorting(id,sorting){
    ...
    if (!isNaN(newSorting) && newSorting.length <= 10){
    	var url = "doNewDailyUpdateSorting.do?productRecommendPojo.id="+id+"&productRecommendPojo.sorting="+newSorting;	
