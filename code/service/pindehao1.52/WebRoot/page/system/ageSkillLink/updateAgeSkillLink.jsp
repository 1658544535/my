...
<body>
	<div class="sub_wrap">
		<div class="s_nav">
			<a href="#">联动标签管理</a> &gt; <a href="#">更新标签</a>
		</div>
		<div class="h15"></div>
		<div>
			<form action="updateAgeSkillLink.do" method="post" id="from1"
				enctype="multipart/form-data">
				<table width="100%" border="0" cellpadding="0" cellspacing="0"
					class="info_table">
						<input type="hidden" name="ageSkillLinkPojo.id" value="${ageSkillLinkPojo.id}" class="floatLeft" />
					<tr>
						...
						<td><select id="ageType"
							name="ageSkillLinkPojo.ageId" class="floatLeft">
								...
								<c:forEach items="${ageTypeList}" var="ageTypeList">
									<option value="${ageTypeList.id}" <c:if test="${ageSkillLinkPojo.ageId == ageTypeList.id}"> selected="selected"</c:if>>${ageTypeList.name}</option>
								</c:forEach>
						</select></td>
						...
						<td><select id="skillType"
							name="ageSkillLinkPojo.skillId" class="floatLeft">
								...
								<c:forEach items="${skillList}" var="skillList">
									<option value="${skillList.id}" <c:if test="${ageSkillLinkPojo.skillId == skillList.id}"> selected="selected"</c:if>>${skillList.name}</option>
								</c:forEach>
						</select></td>
						...
						<td><select id="secondSkillType"
							name="ageSkillLinkPojo.secondSkillId" class="floatLeft">
								...
								<c:forEach items="${secondSkillList}" var="secondSkillList">
									<option value="${secondSkillList.id}" <c:if test="${ageSkillLinkPojo.secondSkillId == secondSkillList.id}"> selected="selected"</c:if>>${secondSkillList.name}</option>
								</c:forEach>
						</select></td>
						...
						<td><select id="productType"
							name="ageSkillLinkPojo.productId" class="floatLeft">
								...
								<c:forEach items="${productTypeList}" var="productTypeList">
									<option value="${productTypeList.id}" <c:if test="${ageSkillLinkPojo.productId == productTypeList.id}"> selected="selected"</c:if>>${productTypeList.name}</option>
								</c:forEach>
						</select></td>
