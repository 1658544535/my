...
<div class="sub_wrap">
  ...
  <div class="h15"></div>
  <div>
  <form action="updatePopup.do" method="post" id="from1" enctype="multipart/form-data">
    <table width="100%" border="0" cellpadding="0" cellspacing="0" class="info_table">
    <input type="hidden" name="popupPojo.id" id="shopLatitude" value="${popupPojo.id}" class="inputText" />
	...
	<tr>
        ...
        <td width="35%">
        <s:if test="popupPojo.popupPic != null"><img src="<s:i18n name="sysconfig"><s:text name="houtai_dns" /></s:i18n>/upfiles/popup/${popupPojo.popupPic}" height='100px'/></s:if>
    ...
    <tr>
       ...
       <td><select name="popupPojo.parameterSize" id="popupPojo.parameterSize"  class="floatLeft">
							<option value="1" <s:if test="popupPojo.parameterSize==1">selected="selected"</s:if>>产品ID</option>
							<option value="2" <s:if test="popupPojo.parameterSize==2">selected="selected"</s:if>>店铺ID</option>
							<option value="3" <s:if test="popupPojo.parameterSize==3">selected="selected"</s:if>>产品类型ID</option>
							<option value="4" <s:if test="popupPojo.parameterSize==4">selected="selected"</s:if>>URL</option>
				    		</select></td>
    ...
    <tr>
        ...
        <td><select name="popupPojo.status" id="popupPojo.status"  class="floatLeft">
							<option value="0" <s:if test="popupPojo.status==0">selected="selected"</s:if>>未通过</option>
							<option value="1" <s:if test="popupPojo.status==1">selected="selected"</s:if>>已通过</option>
				    		</select></td>
