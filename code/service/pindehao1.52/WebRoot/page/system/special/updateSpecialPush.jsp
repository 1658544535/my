...
<div class="sub_wrap">
  ...
  <div class="h15"></div>
  <div>
  <form action="updateSpecialPush.do" method="post" id="from1" enctype="multipart/form-data">
  	<input type="hidden" name="specialPushPojo.id" value="${specialPushPojo.id}" class="floatLeft" id="ticketName" />
	...
	<tr>
        ...
        <td><select name="specialPushPojo.status" id="specialPushPojo.status"  class="floatLeft">
							<option value="0" <s:if test="specialPushPojo.status==0">selected="selected"</s:if>>未审核</option>
							<option value="1" <s:if test="specialPushPojo.status==1">selected="selected"</s:if>>已审核</option>
		                    </select></td>
