...
<script type="text/javascript">
	...
	function del(val) {
		if (confirm("你真的想删除么？")) {
			...
			var url = "delExternalLinks.do?externalLinksPojo.id=" + val;
			doOpreator(url, null);
		} else {
			return;
		}
	...
	function goRefreshPage(result) {
		...
		if (result =="1") {
			...
			queryData("getExternalLinksCount.do", "externalLinksAllList.do?randquery=" + rand);
		} else {
			alert("删除失败");
		}
	}
...
<body>
	<div class="sub_wrap">
		<div class="s_nav">
			<a href="#">活动管理</a> &gt; <a href="#">外链</a>
		</div>
			<form action=goExternalLinks.do method="post" id="sysform">
		...
		<div>
			...
			<a class="Add_btn" href="goAddExternalLinks.do">新增外链</a></s:if>
			<form action="externalLinksDeleteId.do" id="idform" method="post">
...
<script type="text/javascript">
		...
		$("#body")
				.append(
						"<tr><td><input  name='tids' type='checkbox' value="+this.id +"   /></td><td>"
								...
								+ this.id
								+ "')>删除</a>"
								+ "<a class='edit_btn' href='goUpdateExternalLinks.do?externalLinksPojo.id="
	...
	$(function() {
		...
		MAOWU.page.init(<s:property value="page.rowCount"/>,
				"externalLinksAllList.do?randIni=" + rand);
