...
function allcb() {
	...
	function goRefreshPage(result){
		...
		if(result=="1"){
			alert("删除成功");
			queryData("getCountAllProductType.do?pid=${pid}", "getAllProductTypeByPid.do?pid=${pid}&randquery="+rand);
		}else{
			alert("删除失败");
		}
	}
...
<div class="sub_wrap">
  ...
  <div class="h15"></div>
   <div>
   <a href="goproductTypeAdd.do?productTypePojo.pid=-1&productTypePojo.level=1" class="Add_btn">新增分类</a>
...
/* function query() {
	if(tt.validate()){
		var rand=Math.random() * ( 100000 + 1);
		queryData("getCountAllProductType.do?pid=${pid}", "getAllProductTypeByPid.do?pid=${pid}&ran
	}
} */
	...
	function installPage() {
		var _visable = this.visable;
		var visstr = "";
		if(_visable == 1){
			visstr = "<a  class='edit_btn' onclick=invisable('"+this.id+"')>不显示</a>";
		} else if(_visable == 0){
			visstr = "<a  class='edit_btn' onclick=visable('"+this.id+"')>显示</a>";
		}
		$("#body").append(
				"<tr><td><input  name='tids' type='checkbox' value="+this.id +"   /></td>"+
				//"<td>"+ this.userName + "</td>"+
				"<td>"+ this.name + "</td>"+
				"<td><a href='productTypeManage.do?pid="+this.id+"&level="+this.level+"&typeName="+this.name+" > '>查看子类</a></td>"+
				//"<td>"+ this.isRedName + "</td>"+
				"<td>"+ this.statusName + "</td>"+
				"<td>"+ this.visableName + "</td>"+
				"<td>"+this.sorting+"</td>"+
				"</td><s:if test="#session.role.roleId!=7"><td>"+
				//"<a class='edit_btn' href='goproductTypeAdd.do?productTypePojo.pid="+this.id+"&productTypePojo.userId="+this.userId+"&pid="+this.pid+"'>添加子类型</a>" +
				"<a  class='edit_btn' onclick=check('"+this.id+"')>审核</a>"+
				visstr+
				//"<a  class='del_btn' onclick=del('"+this.id+"')>删除</a>"+
				"<a class='edit_btn' href='goFindProductType1.do?productTypePojo.id="+this.id +"'>编辑</a></td></s:if></tr>");
	}
	...
	$(function() {
		...
		MAOWU.page.init(<s:property value="page.rowCount"/>,
				"getAllProductTypeByPid.do?pid=${pid}&randIni="+rand);
