...
tt.i18LenMin = "{0}只能输入{1}个字符，已超出{2}个字符!";
tt.i18Len = "{0}最少要输入{1}个字符，还需要输入{2}个字符!";//{0}的长度不应大于3! //{0}的长度不应小于3!
tt.i18LenExp = "{0}长度合法范围：{1}!";
tt.i18LenTip = "已输入{0}个字符，还可以输入{1}个字符!";

tt.i18Num = "{0}必须为数字!";
tt.i18Regex = "{0}{1}";
tt.i18Ip = "{0}IP错误!";
tt.i18Postcode = "{0}格式错误!";
tt.i18Tel = "{0}正确格式为：0734-1234567或 021-12345678!";
tt.i18Mobile = "手机号码错误!";
tt.i18Idcard = "{0}身份证号码错误!";

tt.i18SelectCountMin = "{0}最多只能选中{1}项，已多选{2}项!";
tt.i18SelectCount = "{0}最少要选中{1}项，还需选择{2}项!";
...
tt.tip = tt.bh.ext({
	h:function()
	{
		...
		if (this.needHandle() && tipMsg) {
			this.render(tt.Conf.tipCls, tipMsg, "talentClose talentCloseTip");
		}
	}
});
/**
...
addStar : function(f) {
	for (var i = 0; i < f.es.length; i++) {
		tt.insertAfter(f.es[i], tt.getI18S("<span id='{0}' class='{1}'>*</span>", [f.es[i][tt.Conf.proNameOfReqStarId], tt.Conf.reqStarCls]));
	}
},
/**
 * 清空星号
...
tt.CV = tt.BV.ext({
	...
	set : function(cmpType, oper, fOrV, showCmpVal) {
		...
		} else if (cmpType == 'n' && this.cmpF !=null) {
			this.i18n = tt.i18NumCompare;
		} else if (cmpType == 'v' && this.cmpF !=null && this.cmpF != '') {
			this.i18n = tt.i18StrCompare;
		} else if (cmpType == 'n' && this.cmpV != null) {
			this.i18n = tt.i18NumValueCompare;
		} else if (cmpType == 'v' && this.cmpV != null && this.cmpV != '') {
...
tt.VF = tt.C.ext({
	init : function() {
		...
		this.mob =  new tt.RV().set(/^13[0-9]{9}|15[012356789][0-9]{8}|18[0256789][0-9]{8}|147[0-9]{8}$/).setI18(tt.i18Mobile);
