...
(function(a){
	a.fn.jdSlide=function(k){
		...
		var n=p.pics.length;
		var o=function(){
			var q="<ul style='position:absolute;top:0;left:0;'><li><a href='shopDetailsWeb.do?shop.id="+p.pics[0].shopId+"' target='_blank'><img src='"+p.pics[0].src+"' width='"+p.width+"' height='"+p.height+"' /></a></li></ul>";
			i.css({
				position:"relative"
			}).html(q);
			i.find("ul").css({
		...
		var j=function(){
			...
			i.find(".o-control").show()
			//i.bind("mouseover",function(){
			//	i.find(".o-control").show()
			//}).bind("mouseleave",function(){
				//i.find(".o-control").hide()
			//})
		};
		var l=function(r,q){
			if(r==n){
		...
		var c=function(){
			...
			for(var r=1;r<n;r++){
				q.push("<li>");
				q.push("<a href='shopDetailsWeb.do?shop.id="+p.pics[r].shopId+"' target='_blank'><img src='");
				q.push(p.pics[r].src);
				q.push("' width='");
				q.push(p.width);
				q.push("' height='");
