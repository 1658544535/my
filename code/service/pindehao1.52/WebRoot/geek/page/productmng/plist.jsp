	...
	<head>
		...
		<script type="text/javascript">
			...
			function goRefreshPage(result){
				...
				if(result=="1"){
					alert("删除成功");
					queryData("productAllCountGeekWeb.do?productPojo.userId=${productPojo.userId}", "getProductAllListGeekWeb.do?productPojo.userId=${productPojo.userId}&randdelete="+rand,pageSize);
				}else{
					alert("删除失败");
				}
			}
					...
					<div class="sp-body">
						...
						<div class="ui-table-container p20">
							<div class="ui-tab m-s-info" style="position: relative">
								<form action="getProductAllListGeekWeb.do" method="post" id="sysform">
							...
							<div class="ui-fn clearfix">
								<a class="ui-button ui-button-lgreen fl" href="goProductAddGeekWeb.do"><i class="iconfont">&#xf0175;</i>添加商品</a>
...
<script type="text/javascript">
	...
	function query() {
		var rand=Math.random() * ( 100000 + 1);
		queryData("productAllCountGeekWeb.do", "getProductAllListGeekWeb.do?randquery=" + rand,pageSize);
	}
	...
	**/
	function installPage() {
		...
		"<td width='80px'><a target='_blank' href='goFindProductGeekWeb.do?productPojo.id="+ this.id + "'>编辑</a><br>"+
		//"<a href='javascript:;' onclick=del('"+ this.id + "')>删除</a><br>"+
		//"<a target='_blank' href='productFocusImagesGeekWeb.do?productFocusImages.productId="+this.id+"' class='edit_btn' )>焦点图片</a><br>"+
		"<a target='_blank' href='productImagesManageGeekWeb.do?productImagesPojo.productId="+this.id+"' class='edit_btn' )>商品图片</a><br>"+
		"<a target='_blank' href='goPreviewGeekWeb.do?productPojo.id="+this.id+"' class='edit_btn' )>商品预览</a>"+
	...
	$(function() {
		/** 首次要初始化分页**/
		var rand=Math.random() * ( 100000 + 1);
		MAOWU.page.init(<s:property value='page.rowCount'/>, "getProductAllListGeekWeb.do?randIni="+rand,pageSize);
