                    ...
                    <div class="pure-u-1 main">
                        ...
                        <h1 class="seller-title">
                            ...
                            <div class="seller-title-head-more">
                                <a class="ui-button ui-button-morange view-ShowBrandBoxBtn" href="goAddUCPostGeek.do">
...
<script type="text/javascript">
	...
	function query() {
		if(tt.validate()){
			...
			queryData("getUserCirclePostListCountGeek.do", "getUserCirclePostListDataGeek.do?randquery="+rand,pageSize);
		}
	}
	
	function installPage() {
		var statusStr = "";
		if(this.statusName == "未审核"){
			statusStr = "审核中"
		}else{
			statusStr = this.statusName;
		}
		$("#body").append(
				"<tr id='"+this.id+"'>"+
                "<td>"+this.title+"</td>"+
                ...
                "<td>"+statusStr+"</td>"+
                "<td class='ui-table-action'><a target='' href='goUpdateUCPostGeek.do?userCirclePostPojo.id="+this.id+"'>编辑</a>&nbsp;&nbsp;&nbsp;&nbsp;<a target='' href='delUCPost.do?userCirclePostPojo.id="+this.id+"' onclick='javascript:return p_del()'>删除</a></td>"+
                "</tr>"
                );
	}
	//分页展现页面函数 
	$(function() {
		//首次要初始化分页
		var rand=Math.random() * ( 100000 + 1);
		MAOWU.page.init(<s:property value="page.rowCount"/>,"getUserCirclePostListDataGeek.do?randIni="+rand,pageSize);
