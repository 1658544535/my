...
<script type="text/javascript">
	...
	function del(val) {
		if (confirm("你真的想删除该记录么？")) {
			var url = "delSecurityQuesion.do?accountSecurityPojo.user_id=" + val;
			doOpreator(url, null);
		} else {
			return;
		}
	...
	function check(val) {
		if (confirm("你确定要审核该记录么？")) {
			var url = "verifySecurityQuesion.do?accountSecurityPojo.user_id=" + val;
...
<body>
	<div class="sub_wrap">
			...
			<form action=accountSecurityManage.do method="post" id="sysform">
				<table width="100%" border="0" class="Search_table">
					<tr align="center">
						...
						<td align="left">
						<select name="accountSecurityPojo.status" id="">
						 		<option value="">全部</option>
								<s:iterator value="securityQuesionTypeList">
									<option value="<s:property value="value"/>"
									<s:if test="value == accountSecurityPojo.status">selected="selected"</s:if>><s:property
											value="name" />
									</option>
								</s:iterator>
						</select>
		...
		<a class="Add_btn" onclick="checkAll()">批量审核</a>
			<form action="verifyAllSecurityQuesion.do" id="idform" method="post">
