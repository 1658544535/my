...
<body>
	<div class="sub_wrap">
		<div class="s_nav"><a href="#">拼得客邀请用户列表</a> &gt; <a href="#">拼得客邀请用户列表</a></div>
		...
		<input name="thirtyDayStr" id="thirtyDayStr" type="hidden" placeholder="" class="floatLeft" />
...
<script type="text/javascript">
	...
	function query() {
		if(tt.validate()){
			...
			queryData("pindekeMonthSaleCntInviter.do", "pindekeMonthSaleListInviter.do?randquery="+rand);
		}
	}
	
	function settleAll(url){
		if(confirm("确认要返佣吗？")){
			var formParam = $("#idform").serialize();
			MAOWU.ajax.get(url, formParam, settleRefreshPage);
		}else{
			return ;
		}
	}
	...
	function installPage() {
		...
		if(this.isSettle == 0 && this.total >= 1000){
			isSellteName = "未返佣";
			option = "<a class='edit_btn' onclick='settle(\"settlePindekeMonthSale.do?id="+this.id +"&pindekeMonthSalePojo.userId="+this.userId+"&pindekeMonthSalePojo.settleAmt="+this.settleAmt+"\")'>点击返佣</a>";
		}else if(this.isSettle == 1 && this.total >= 1000){
			isSellteName = "已返佣";
		}else{
			isSellteName = "无返佣";
		}
	...
	$(function() {
		var rand=Math.random() * (100001);
		MAOWU.page.init(${page.rowCount}, "pindekeMonthSaleListInviter.do?randIni="+rand);	
