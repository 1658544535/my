...
<script type="text/javascript">
	...
	var ddd=this.status;
	var aaa="";
	if(ddd == 3 && (this.isRefund != 1 || this.isRefund != 2)){
	   var ss = "退货中";
	   aaa="<a class='edit_btn' href='checkOrderRefund.do?userOrderRefundPojo.id="
			+ this.id + "&userOrderRefundPojo.orderId="+this.orderId+"&userOrderRefundPojo.detailId="+this.detailId+"&is=4&orderId="+this.orderId+"&payMethod="+this.payMethod+"&pageNoVal="+pageNoVal+"&formParam="+formParam+"'>确认退货退款</a>";
	}else if(ddd == 2 && (this.isRefund != 1 || this.isRefund != 2)){
	   var ss = "买家填写退货信息中";
	}
	var uncheck = "";
	if(this.isRefund != 2 || this.isRefund != 3){
		uncheck += "<a class='edit_btn' href='checkOrderRefund.do?userOrderRefundPojo.id="
		+ this.id + "&userOrderRefundPojo.orderId="+this.orderId+"&userOrderRefundPojo.detailId="+this.detailId+"&is=5&pageNoVal="+pageNoVal+"&formParam="+formParam+"'>不确认退货退款</a>"
	}
	var types=this.type;
	if(types==1){
		var typed="退款";
	...
	var h=this.stockPrice*this.refundNum-this.couponPrice;
		...
		$("#body")
				.append(
						trStr+"<td><input  name='tids' type='checkbox' value="+this.id +"   /></td><td>"
								+ typed
								+ "</td><td>"
								+ this.loginname1
								+ "</td><td>"
							...
							<!--	+ "</td><td>"
								...
								//+ "</td>"
								+"<s:if test="#session.role.roleId==12 || #session.role.roleId==13 || #session.role.roleId==1"><td>"
								+ aaa
								+ uncheck
								...
								+ this.id + "&userOrderRefundPojo.orderId="+this.orderId+"&userOrderRefundPojo.detailId="+this.detailId+"&pageNoVal="+pageNoVal+"&formParam="+formParam+"'>详情</a>"
							//	+ "<a class='edit_btn' href='queryDeliverInfo.do?logtype=" + this.logType + "&lognum="+this.logistics+"'>查询物流</a>"
								+"</td></s:if>"
								+"<s:if test="#session.role.roleId!=12 && #session.role.roleId!=13 && #session.role.roleId!=1"><td>"
								+ "<a class='edit_btn' href='goFindRefundOrder.do?userOrderRefundPojo.id="
								+ this.id + "&userOrderRefundPojo.orderId="+this.orderId+"&userOrderRefundPojo.detailId="+this.detailId+"&pageNoVal="+pageNoVal+"&formParam="+formParam+"'>详情</a>"
