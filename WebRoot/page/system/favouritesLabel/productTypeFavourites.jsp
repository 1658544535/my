...
function del(val){
	...
	function goRefreshPage(result){
		...
		if(result=="1"){
			alert("删除成功");
			queryData("userCirclePostFavouritesProTypeCount.do", "userCirclePostFavouritesList.do?randquery="+rand);
			$("a[name='addButton']").attr('style','');
		}else{
			alert("删除失败");
			$("a[name='addButton']").attr('style','display: none;');
		}
	}
	</script>
</head>
<body>
<div class="sub_wrap">
<div class="s_nav"><a href="#">标签管理</a> &gt; <a href="favouritesLabelList.do">有你喜欢</a> &gt; <a href="#">产品分类</a></div>
  ...
  <div class="h15"></div>
  <div>
<a class="Add_btn" onclick="addProductType(${yourFavouritesPojo.id })" name="addButton">添加</a>
...
function addProductType(id){
	var msg = "请输入产品分类ID！~";
	var pid = prompt(msg, "");
    if (!isNaN(pid) && pid.length <= 20){
    	var url = "doAddProductTypeId.do?yourFavouritesDetailPojo.favId="+id+"&yourFavouritesDetailPojo.productTypeId="+pid;	
		doupdateSorting(url,null);
    }else{
    	alert("您输入的产品类型ID格式不正确！~请重新输入！~");
    }
}
