...
<body>
	<div class="sub_wrap">
		<div class="s_nav">
			<a href="#">系统管理</a> &gt; <c:if test="${os==2}"><a href="#">供应商管理</a></c:if> <c:if test="${os==3}"><a href="#">采购商管理</a></c:if>
		</div>
		...
		<form action="sysLoginCS.do?os=${os}" method="post" id="sysform">
			...
			<div id="search_show" style="">
				<table width="100%" border="0" class="Search_table">
					<tr>
						...
						<td><label><input type="text" name="sysLogin.QQ"
								id="ticketRulePojo.QQ"
								value="<s:property value="sysLogin.QQ"/>"></label></td>
					</tr>
					<!--
					<tr>
						<td align="right">用户ID：</td>
						<td><label><input type="text" name="sysLogin.id"></label></td>
					</tr>
					-->
				</table>
		...
		<div class="h15"></div>
		<div>
			<s:if test="#session.role.roleId!=2"><a class="delAll_btn" onclick="deleteAll()">审核全部</a><a href="addSysLogin.do?os=${os}" class="Add_btn">新增用户</a></s:if>
	...
	function installPage() {
		$("#body").append(
			...
			<!--	"</td><td>" + this.id + -->
				...
				+ "</td><td>" + this.consumerdate
				+ "</td><td>" + this.login_date
				+ "</td>"+
				"<td><!--<a  class='edit_btn' onclick=del('"+this.id+"')>审核</a>-->"
				...
				+ "<a class='edit_btn' href='goFindConsumer.do?consumer.userId="+this.id +"&os=<s:property value='os'/>"+"'>账户信息</a>"
				+ <s:if test="#session.role.roleId!=2">
				...
				+ "<!--<a class='del_btn' href='delSysLogin.do?sysLogin.id="+this.id+"&os=<s:property value='os'/>"+"' onclick='javascript:return window.confirm(\"确定删除？(该用户账户信息将被删除)\");'>删除用户</a>--></td>"+
				</s:if>
				"</tr>");
	}
