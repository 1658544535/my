...
<body>
	<div class="sub_wrap">
		...
		<form action="sysLoginMF.do?os=${os}" method="post" id="sysform">
		...
		<s:if test="#session.role.roleId!=7">
			<a class="delAll_btn" onclick="deleteAll()">批量审核</a>
			<a href="addSysLogin.do?os=${os}" class="Add_btn">新增用户</a>
			<a href="genExternalCode.do?os=${os}" class="Add_btn">生成邀请码</a></s:if>
	...
	function installPage() {
		$("#body").append(
		   ...
		   <!-- "</td><td>"+this.id +    -->
				...
				//+ "<a class='edit_btn' href='goFindManufacturer.do?manufacturer.userId="+this.id +"&os=<s:property value='os'/>"+"'>账户信息</a>"
				...
				<!--<c:if test="${os==2}">-->
				<!--+ "<a class='edit_btn' href='addManufacturerInfoAndShop.do?manufacturer.userId=" + this.id +"'>品牌新增</a>"-->
				<!-- </c:if> -->
			   <!-- + "<a class='del_btn' href='delSysLogin.do?sysLogin.id="+this.id+"&os=<s:property value='os'/>"+"' onclick='javascript:return window.confirm(\"确定删除？(该供应商账户信息将被删除)\");'>删除用户</a>"-->
				+"</td></s:if></tr>");
	}
