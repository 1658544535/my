...
<body>
	<div class="sub_wrap">
		<div class="s_nav"><a href="#">拼得客任务</a> &gt; <a href="#">拼得客任务团列表管理</a></div>
...
var pagecount = "${page.rowCount}"; 
	...
	function query() {
		if(tt.validate()){
			...
			queryData("pindekeTaskGroRecCount.do?grouponUserRecordPojo.id=${grouponUserRecordPojo.id}", "pindekeTaskGroRecList.do?grouponUserRecordPojo.id=${grouponUserRecordPojo.id}&randquery="+rand+"");
		}
	}
	...
	function installPage() {
		...
		$("#body").append(
				...
				"<td>"+ statusStr + "</td>"+
				"<td><a class='edit_btn' href='goPindekeTaskGroUserRec.do?grouponUserRecordPojo.attendId="+this.id+"&grouponUserRecordPojo.activityStatus="+this.activityStatus+"&grouponUserRecordPojo.isPrize="+this.isPrize+"&grouponUserRecordPojo.activityId=${grouponUserRecordPojo.id}'>查看</a></td>"+
				"</tr>"
				);
	}
	/**
	...
	$(function() {
		var rand=Math.random() * (100001);
		MAOWU.page.init("${page.rowCount}", "pindekeTaskGroRecList.do?grouponUserRecordPojo.id=${grouponUserRecordPojo.id}&randIni="+rand+"&groActId="+"${id}");	
